...
const preferences = [{
  ...
  field: "study_habits",
  icon: BookOpen,
  options: ["Library Lover", "Room Studier", "Café Goer"]
}, {
  label: "Smoking/Drinking",
  field: "smoking_drinking",
  icon: Wine,
  options: ["Neither", "Social Only", "Regular"]
}];
const ProfileSetup = () => {
  ...
  return <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      y: 0
        ...
        <div className="flex items-center gap-2 mb-8">
          {steps.map((s, i) => <div key={s} className="flex items-center gap-2 flex-1">
        ...
        <div className="bg-card rounded-2xl p-8 shadow-card">
          ...
          <AnimatePresence mode="wait">
            {step === 0 && <motion.div key="step0" initial={{
            opacity: 0,
            x: 20
          }} animate={{
            opacity: 1,
            ...
            x: -20
                ...
                <div><Label>College / University</Label><Input className="mt-1" placeholder="e.g. MIT" value={info.college} onChange={e => setInfo(f => ({
                ...f,
                college: e.target.value
              }))} /></div>
                <div><Label>Course / Major</Label><Input className="mt-1" placeholder="e.g. Computer Science" value={info.course} onChange={e => setInfo(f => ({
                ...f,
                course: e.target.value
              }))} /></div>
                <div><Label>Year</Label><Input className="mt-1" placeholder="e.g. Sophomore" value={info.year} onChange={e => setInfo(f => ({
                ...f,
                year: e.target.value
              }))} /></div>
              </motion.div>}
            ...
            x: -20
                ...
                {preferences.slice(0, 2).map(p => <div key={p.field}>
            ...
            x: -20
                ...
                {preferences.slice(2).map(p => <div key={p.field}>
                    <div className="flex items-center gap-2 mb-3">
                      <p.icon className="w-5 h-5 text-primary" />
                      <Label className="text-base">{p.label}</Label>
                    </div>
