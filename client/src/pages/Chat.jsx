  ...
  useEffect(() => {
    ...
    api.get(`/api/users/${receiverId}`).then(data => {
      if (data?.name) {
        ...
        setOtherInitials(data.name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2));
      }
    }).catch(err => {
    ...
    if (socket) {
      ...
      socket.on("messages_read", data => {
        if (data.readerId === receiverId) {
          setMessages(prev => prev.map(m => m.sender === user._id ? {
            ...m,
            isRead: true
          } : m));
        }
  ...
  return <div className="min-h-screen bg-background flex flex-col">
      <header className="sticky top-0 z-50 bg-card/90 backdrop-blur-md border-b border-border">
        <div className="container mx-auto flex items-center gap-3 h-16 px-4">
          ...
          <div className="flex-1">
            ...
            {isOtherTyping ? <p className="text-xs text-primary font-medium animate-pulse">typing...</p> : <p className="text-xs text-success font-medium">Online</p>}
          </div>
        </div>
      </header>

      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-3">
        {messages.length === 0 && <div className="text-center py-16 text-muted-foreground">
            <p>No messages yet. Say hello! 👋</p>
          </div>}
        {messages.map((m, i) => <motion.div key={m._id} initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        ...
        delay: Math.min(i * 0.03, 0.3)
            ...
            <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${m.sender === user?._id ? "bg-primary text-primary-foreground rounded-br-md" : "bg-card shadow-card text-card-foreground rounded-bl-md"}`}>
              {m.imageUrl && <div className="relative rounded-lg overflow-hidden mb-2 max-w-sm border border-border/10 bg-black/5">
                  <img src={m.imageUrl} alt="Attachment" className="max-h-60 w-full object-cover transition-transform hover:scale-105 duration-300" loading="lazy" />
                </div>}
              {(!m.imageUrl || m.content !== "Sent an image") && <p className="text-sm leading-relaxed whitespace-pre-wrap break-words">{m.content}</p>}
              <div className={`flex items-center gap-1 mt-1 ${m.sender === user?._id ? "justify-end" : ""}`}>
                ...
                {m.sender === user?._id && (m.isRead ? <CheckCheck className="w-3 h-3 text-primary-foreground/60" /> : <Check className="w-3 h-3 text-primary-foreground/60" />)}
              </div>
            </div>
          </motion.div>)}

        {isOtherTyping && <motion.div initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        y: 0
            ...
            <div className="bg-card shadow-card text-card-foreground rounded-2xl rounded-bl-md px-4 py-3 flex items-center gap-1">
              ...
              <span className="w-1.5 h-1.5 bg-muted-foreground/60 rounded-full animate-bounce" style={{
            animationDelay: "0ms"
          }}></span>
              <span className="w-1.5 h-1.5 bg-muted-foreground/60 rounded-full animate-bounce" style={{
      ...
      <div className="sticky bottom-0 bg-card/90 backdrop-blur-md border-t border-border p-4">
        <div className="container mx-auto flex items-center gap-2">
          ...
          <Button variant="ghost" size="icon" onClick={handleImageClick} disabled={isUploading}>
          ...
          <Input placeholder="Type a message..." className="flex-1" value={input} onChange={e => handleInputChange(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} />
