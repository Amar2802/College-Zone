...
function calcStudyMatch(me: Profile, other: Profile): number {
...
type Tab = "matches" | "study-buddies" | "events" | "chats" | "profile";
...
const Dashboard = () => {
  ...
  const [studyBuddies, setStudyBuddies] = useState<Profile[]>([]);
      ...
      try {
        ...
        if (user.profile?.college) {
          try {
            ...
            const mappedBuddies: Profile[] = buddies.map((u: any) => ({
              id: u._id,
              user_id: u._id,
              full_name: u.name,
              college: u.profile?.college || "",
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const eventWithOrganizer = {
        ...newEvent,
        organizer: { _id: user?._id, name: user?.name }
      };
  ...
  // Roommate Filtering
  const roommateMatches = profiles
    .map(p => ({
      ...p,
  ...
  const filteredRoommates = roommateMatches.filter(m =>
    m.full_name.toLowerCase().includes(search.toLowerCase()) ||
    m.course.toLowerCase().includes(search.toLowerCase())
  );

  // Study Buddies Filtering
  const studyBuddyMatches = studyBuddies
    .map(p => ({
      ...p,
  ...
  const filteredStudyBuddies = studyBuddyMatches.filter(m =>
    m.full_name.toLowerCase().includes(search.toLowerCase()) ||
    m.course.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
      ...
      <main className="container mx-auto px-4 py-6 pb-24">
        ...
        {tab === "study-buddies" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            {!myProfile?.college ? (
              <div className="bg-card rounded-2xl p-6 shadow-card text-center py-12 border border-dashed border-border">
        ...
        {tab === "events" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            {!myProfile?.college ? (
              ...
              <>
                {events.length === 0 ? (
                  <div className="text-center py-16 bg-card rounded-2xl p-6 shadow-card border border-dashed border-border">
                        ...
                        >
                          <div>
                            ...
                            <div className="space-y-2 mb-4 text-xs text-muted-foreground font-medium">
                              <div className="flex items-center gap-2">
                                ...
                                <span>{new Date(e.date).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</span>
                              </div>
                              <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-border mt-auto">
                            ...
                            <Button
                              ...
                              size="sm"
                              className={isAttending ? "bg-success/15 text-success hover:bg-success/20 border-transparent" : ""}
                              onClick={() => handleRSVP(e._id)}
                            >
                              {isAttending ? (
                                <span className="flex items-center gap-1 font-bold"><Check className="w-3.5 h-3.5" /> RSVP'd</span>
                              ) : (
                                "Join Event"
                              )}
                            </Button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            {filteredRoommates.length === 0 ? (
              ...
              <div className="space-y-2">
                {filteredRoommates.map((m) => (
                  <div
                    key={m.id}
                    onClick={() => navigate(`/chat?to=${m.user_id}`)}
                    className="flex items-center gap-4 p-4 rounded-2xl bg-card hover:bg-secondary/40 border border-border/50 transition-all duration-200 cursor-pointer"
                  >
            ...
            <div className="bg-card rounded-2xl p-6 shadow-card space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: "Study Habits", value: myProfile.study_habits },
                  { label: "Social Life", value: myProfile.smoking_drinking },
                ].map(p => (
        ...
        {showCreateEventModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              {eventError && (
                <div className="bg-destructive/15 text-destructive text-sm p-3 rounded-lg mb-4 font-medium">
                  {eventError}
                </div>
              )}
              ...
              <form onSubmit={handleCreateEvent} className="space-y-4">
                <div>
                  ...
                  <Input
                    placeholder="e.g. Midterm Prep & Coffee Study Session 📚"
                    value={eventTitle}
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      placeholder="e.g. Central Library Study Room 302"
                      value={eventLocation}
      ...
      <nav className="fixed bottom-0 w-full bg-card/90 backdrop-blur-md border-t border-border z-50">
        <div className="container mx-auto flex justify-around py-2">
          {([
            { key: "matches" as Tab, icon: Users, label: "Roommates" },
            { key: "study-buddies" as Tab, icon: BookOpen, label: "Study Buddies" },
