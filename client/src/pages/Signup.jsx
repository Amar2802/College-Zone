...
const Signup = () => {
  ...
  const handleSignup = async e => {
    ...
    try {
      ...
      signUp(data.token, data);
      toast({
        title: "Account created! Let's set up your profile 🎓"
      });
  ...
  const handleGoogleAuthSuccess = async googleToken => {
    ...
    try {
      ...
      signUp(data.token, data);
      toast({
        title: "Welcome to College Zone! Let's set up your profile 🎓"
      });
      navigate("/profile-setup");
    } catch (error) {
      toast({
        title: error.message || "Google signup failed",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  return <div className="min-h-screen bg-background flex">
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-hero items-center justify-center p-12 relative overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          {[...Array(6)].map((_, i) => <div key={i} className="absolute rounded-full bg-primary-foreground" style={{
          width: `${60 + i * 40}px`,
          height: `${60 + i * 40}px`,
          top: `${10 + i * 15}%`,
          right: `${5 + i * 12}%`,
        ...
        y: 0
          ...
          <div className="w-full mb-6 flex justify-center">
            ...
            if (credentialResponse.credential) {
              await handleGoogleAuthSuccess(credentialResponse.credential);
            }
          }} onError={() => {
            toast({
